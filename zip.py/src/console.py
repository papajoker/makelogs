...
RM_SUDO = str(Path.home()).startswith("/root")    # run this script with pkexec ?
...
def store_commands() -> Generator:
    """
        list yaml files and display commands
        return: array for gui
    """
    #result = []
    i = 0
    for yaml in get_files_name():
        ...
        for action in datas['actions']:
            ...
            if '(' in cmd or '?' in cmd:
                cmd = f"'{cmd}'"
            #result.append((service, action['name'], get_action_title(action, lang), False, i, cmd, action.get("command","")))
            yield (service, action['name'], get_action_title(action, lang), False, i, cmd, action.get("command",""))
            i += 1
...
def execute():
    ...
    """ TODO
        best is to pass log_filename as parameter to script
        -f "/home/me/xxx/logs.md"
    """
    if RM_SUDO:
        # ??? path.cwd return /root  with pkexec ???
        log_filename = Path(configdir).parent / "logs.md"
        print("New log_filename", log_filename)
    ...
    print(f"\nRead '{log_filename}' file, and after, you can send to coud by './makelogs -s'")
